    ...
    public partial class UserController
    {
        ...
        public IActionResult GetInitiatedApplications(int pageIndex = 0, int pageSize = 10)
        {
            ...
            foreach (var application in applications)
            {
                ...
                var Corrigendum = dbcontext.Corrigendum.Where(co => co.ReferenceNumber == application.ReferenceNumber).ToList();
                List<string> corrigendumIds = [];
                var expiringEligibility = dbcontext.ApplicationsWithExpiringEligibility.FirstOrDefault(aee => aee.ReferenceNumber == application.ReferenceNumber);
